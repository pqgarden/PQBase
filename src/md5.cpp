...
#define ROTATE_LEFT(x, n) (((x) << (n)) | ((x) >> (32-(n))))  //右移的时候，高位一定要补零，而不是补充符号位
...
#define II(a, b, c, d, x, s, ac) { \
	...
	MD5::MD5(const void* input, size_type length) {
		reset();
		update(input, length);
	}
	...
	void MD5::update(ifstream& in) {
		...
		while (!in.eof()) {
			...
			length = in.gcount();
			if (length > 0) {
				update(buffer, (size_type)length);
			}
		}
		in.close();
	}
	...
	*/
	...
	/* Encodes input (ulong) into output (unsigned char).
	Assumes length is a multiple of 4.
	*/
	void MD5::encode(const uint32* input, unsigned char* output, size_type length) {

		for (size_type i = 0, j = 0; j < length; ++i, j += 4) {
	...
	/* Decodes input (unsigned char) into output (ulong).
	Assumes length is a multiple of 4.
	*/
	void MD5::decode(const unsigned char* input, uint32* output, size_type length) {
	...
	string MD5::bytesToHexString(const unsigned char* input, size_type length) {
