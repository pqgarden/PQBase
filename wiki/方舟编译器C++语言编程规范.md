...
| [11 安全编码规范](#c11) | [基本原则](#c11-1) [变量](#c11-2) [断言](#c11-3) [异常机制](#c11-4) [内存](#c11-5) [危险函数](#c11-6) |
...
## <a name="c2-2"></a> 文件命名
### <a name="a2-2-1"></a>建议2.2.1 C++文件以.cpp结尾，头文件以.h结尾。文件名使用下划线小写风格。
...
例外：对于如` static_cast<char*>(somePointer)/(char*)/vector<char*> `中的场景，*与类型之间不加空格。
...
### <a name="r9-2-2"></a>规则9.2.1 switch语句要有default分支
...
### <a name="a10-4-2"></a>在接口层面明确指针不会为`nullptr`

**理由**

1. 避免解引用空指针的错误。
2. 避免重复检查空指针，提高代码效率。建议使用`gsl::not_null`，或参考实现自己的版本（如使用`NullObject`模式）。对于集合，在不违反已有的接口约定的情况下，建议返回空集合而避免返回空指针，当返回字符串时，建议返回空串 `""`。
...
### <a name="r11-5-1"></a>规则11.5.1 内存申请前，必须对申请内存大小进行合法性校验
...
### <a name="r11-5-2"></a>规则11.5.2 内存分配后必须判断是否成功
...
### <a name="r11-6-1"></a>规则11.6.1 禁止使用内存操作类危险函数
C标准的许多函数，没有将目标缓冲区的大小作为参数，并且未考虑到内存重叠、非法指针的情况，在使用中很容易引入缓冲区溢出等安全漏洞。
...
stdin流输入函数：gets() 
请使用对应的安全函数(详细信息请参考huawei_secure_c)。

例外：在下列情况下，由于未涉及到外部数据处理，不存在被攻击的场景，内存操作完全在本函数内完成，不存在因外部控制而失败的可能性。
如果使用安全函数反而造成代码的冗余，可以留用危险函数：
...
void Foo() {
  ...
  ...
  memset(array, c1, sizeof(array)); //对全局固定长度的数据赋值
  ...
...
void Foo() {
  Tag dest;
  ...
  memcpy((void *)&dest, (const void *)&g_tag, sizeof(Tag)); //对固定长度结构体赋值
  ...
}
```
...
（5）源内存全部是静态字符串常量（编码时需要检查目标内存是否足够的存储空间）：
